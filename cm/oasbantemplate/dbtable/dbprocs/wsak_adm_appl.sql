CREATE OR REPLACE PACKAGE BANINST1.wsak_adm_appl IS
  ...
  --                                                         these can be used for other applications, e.g. Bisk or Into besides VZ.
	-- 	 H       OASBAN-141     09/11/2013        VBANGALO     Removed commented out code.
  --
  --****************************************************************************************************************************
  ...
  */
  ...
  BEGIN
    ...
    p_log('conf number is ' || gv_conf_numb);
		-- wf_get_facts_app_name_db returns appid as TA_XXXXx created for Transient application. For applications
		-- received through web service, like bisk, vz, etc.. TA is replaced with corresponding application specific
		-- app id. Eg: for BISK, TA_123456 is replaced by BA_123456.
  ...
  (
   ...
   ,pv_message_o   OUT VARCHAR2
    ...
    lv_cnt        NUMBER := 0;
    lv_records    NUMBER := 0;
    lv_docid_prefix     swrvdoc.swrvdoc_conf_numb%TYPE := NULL;
    lv_vz_conf_numb     swrvdoc.swrvdoc_conf_numb%TYPE := NULL;
      ...
      LOOP
        BEGIN
          ...
          /*Version G.2 - Check for the document ID prefix of VZ or BA*/
            -- substring file entry to get Vz-Id
            lv_vz_conf_numb := substr(substr(lv_line, (instr(lv_line, '\VZ', 1, 1) + 1),
                                            (instr(lv_line, ' ', 1, 2) -
                                             instr(lv_line, '\VZ', 1, 1))), 1,
                                instr(substr(lv_line, (instr(lv_line, '\VZ', 1, 1) + 1),
                                            (instr(lv_line, ' ', 1, 2) -
                                             instr(lv_line, '\VZ', 1, 1))), '_', 1, 1) - 1);
            --
            IF SUBSTR(lv_line, 1, 7) = 'SUCCESS' AND SUBSTR(lv_vz_conf_numb, 1, 2)  = 'VZ' THEN
                lv_docid_prefix := 'VZ';
            ELSE
